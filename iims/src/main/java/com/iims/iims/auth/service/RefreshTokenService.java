...
public class RefreshTokenService {
    ...
    public RefreshToken createOrUpdateRefreshToken(User user) {
        ...
        if (existingTokenOpt.isPresent()) {
            ...
            refreshToken.setToken(UUID.randomUUID().toString()); // Generate a new token
            refreshToken.setExpiryDate(Instant.now().plusMillis(604800000L)); // Set a new expiry date (e.g., 7 days)
        } else {
        ...
        return refreshTokenRepository.save(refreshToken); // Save or update the record
    }
